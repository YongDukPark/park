...
public class Paging {
	...
	public String getPagingHtml() {	//list.jsp에서 이 method를 호출하는거다. 해당 변수를 리턴한다.
	...
	public Paging(
		...
		this.totalPage = (int)Math.ceil((double)this.totalCount / this.pageSize) ;
							// ceil 은 무조건 올려라 3.5면 4로 	 (double)7.0 / 3 하면 3.5 = 4 페이지가 나온다.
		...
		//offset 건너뛰는 변수가 들어간다.
		this.offset = ( pageNumber - 1 ) * pageSize ; 	//한페이지당 2개씩 보여주면 선택한페이지가 3페이지면 pageNumber에 3이 들어가며 -1 후 pageSize(2)를 곱한다. 
		if( this.endRow > this.totalCount ){			//3페이지를 보고싶다면 앞에 4개를 건너뛰고 보여준다.2개를 보여준다. 건너뛰는 변수를 offset에 넣는다.
		...
		if( this.endPage > this.totalPage ){	//삭제시 만약 그 페이지가 삭제될거같으면 endpage를 total페이지로 바꿔라
	...
	private String getPagingHtml( String url ){ //페이징 문자열을 만든다.
		...
		if (this.beginPage != 1) { // 앞쪽, pageSize:한 화면에 보이는 레코드 수	시작이 1이 아니면 맨처음 이전을 띄운다.
			result += "&nbsp;<a href='" + url  
